...
When a WebView file covers a page, record it alongside Playwright evidence. Bun.WebView stack-browser tests live under `apps/web/bun-test/browser/` and cover the same routes as Playwright commands in `apps/web/e2e/stack-browser/commands.ts`. Update the checklist when promoting WebView paths into the weighted e2e score.
...
Default `web-octane` stays fast (no Docker auth stubs) and uses bun:test for unit tests (merge gate) with Vitest for `.tsrx` integration residuals. True auth/email path coverage is the separate `e2e-stack` job (now Bun.WebView-based). The `coverage-weighted` job enforces D-QH-02 without reviving component Playwright. Compose dialect health (Traefik `/` + `/health` + `system.db_probe`) is the `compose-smoke` matrix — not folded into `smoke-protocol`. Forge protocol edges (Smart HTTP / SSH TCP / LFS batch / packages PathPrefix) are the `smoke-protocol` job — not happy-dom only.
