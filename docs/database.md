...
Operators choose **SQLite**, **PostgreSQL**, or **MySQL** for application data via `DATABASE_URL` (and optional `OCTANEST_DB_DIALECT`). Canonical reference for Phase 2 multi-DB support.

Local Compose defaults use throwaway credentials `octanest` / `octanest`. **Do not** use those in production — set real credentials and do not expose an instance without auth.

## Supported dialects

| Dialect | Sample `DATABASE_URL` | Bring-up |
|---------|----------------------|----------|
| PostgreSQL (default) | `postgres://octanest:octanest@postgres:5432/octanest` | `make up` then `make smoke` |
| MySQL | `mysql://octanest:octanest@mysql:3306/octanest` | `make up-mysql` then `make smoke-mysql` |
| SQLite | `sqlite:./var/octanest.db` | `make up-sqlite` then `make smoke-sqlite` |
...
- Optional `OCTANEST_DB_DIALECT=postgres|mysql|sqlite` must **agree** with the URL scheme, or the API exits at boot with `does not match DATABASE_URL scheme`.
- `mariadb://` is **not** supported.

## Migrations

- One hand-synced `.sql` set per dialect under `crates/octanest-db/migrations/{postgres,mysql,sqlite}/`.
- Applied by the sqlx migrator with `_sqlx_migrations` bookkeeping.
- `OCTANEST_AUTO_MIGRATE` defaults to `true` in Compose/dev. Set `false` for prod-like deploys and run `make db-migrate`.

## SQLite specifics

- Default file: `./var/octanest.db` (`var/` is gitignored runtime state).
...
2. Point `DATABASE_URL` at the new empty target (and optionally set agreeing `OCTANEST_DB_DIALECT`).
...
Documented `octanest`/`octanest` passwords are **local-only**. Set real credentials and a non-public bind before exposing an instance.
