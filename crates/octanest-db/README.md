...
Multi-dialect persistence for Octanest: SQLx migrations and the `Database` API for **PostgreSQL**, **MySQL**, and **SQLite**.
...
- Called by `octanest-api` — the API must not fork queries per dialect
...
```bash
cargo nextest run -p octanest-db
# or: make test
```
