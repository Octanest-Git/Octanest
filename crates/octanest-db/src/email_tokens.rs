        ...
        let attempt_count = row
            ...
            .or_else(|_| {
                row.try_get::<i64, _>("attempt_count")
                    .map(|v| v as i32)
                    .or_else(|_| row.try_get::<i8, _>("attempt_count").map(|v| i32::from(v)))
            })
...
const TOKEN_SELECT_PG: &str = "SELECT id, user_id, purpose, token_hash, otp_hash, attempt_count,
       to_char(expires_at AT TIME ZONE 'UTC', 'YYYY-MM-DD\"T\"HH24:MI:SS\"Z\"') AS expires_at,
       to_char(created_at AT TIME ZONE 'UTC', 'YYYY-MM-DD\"T\"HH24:MI:SS\"Z\"') AS created_at
FROM auth_email_tokens";

const TOKEN_SELECT_MYSQL: &str = "SELECT id, user_id, purpose, token_hash, otp_hash, attempt_count,
       DATE_FORMAT(expires_at, '%Y-%m-%dT%H:%i:%sZ') AS expires_at,
       DATE_FORMAT(created_at, '%Y-%m-%dT%H:%i:%sZ') AS created_at
FROM auth_email_tokens";

const TOKEN_SELECT_SQLITE: &str = "SELECT id, user_id, purpose, token_hash, otp_hash, attempt_count,
...
(id, user_id, purpose, token_hash, otp_hash, expires_at, attempt_count)
VALUES ($1, $2, $3, $4, $5, $6::timestamptz, 0)
