//! Dialect resolution from `DATABASE_URL` / `OCTANEST_DB_DIALECT` (D-01, D-02).
...
pub fn parse_declared(value: &str) -> Result<Dialect, String> {
    match value {
        ...
        _ => Err(format!(
            "unknown OCTANEST_DB_DIALECT: {value} (expected postgres|mysql|sqlite)"
        )),
    }
}
...
pub fn resolve_dialect_from_env(url: &str) -> Result<Dialect, String> {
    let declared = std::env::var("OCTANEST_DB_DIALECT").ok();
...
mod tests {
    ...
    fn resolve_dialect_schemes() {
        let cases = [
            ...
            ("sqlite:./var/octanest.db", Dialect::Sqlite),
            ("sqlite://./var/octanest.db", Dialect::Sqlite),
    ...
    fn redact_url_hides_password() {
        let redacted = redact_url("postgres://octanest:hunter2@db:5432/octanest");
        assert!(
            !redacted.contains("hunter2"),
            "password leaked: {redacted}"
        );
        assert!(redacted.contains("octanest:***@db:5432/octanest"));
    }
}
