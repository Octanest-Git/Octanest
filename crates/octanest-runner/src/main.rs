//! octanest-runner — self-hosted Actions runner for Octanest's JSON protocol.
//!
    ...
    let st = match state::load(&config.state_path) {
        ...
        None => {
            let reg = match &config.registration_token {
                ...
                None => {
                    eprintln!(
                        "octanest-runner: no state at {} and OCTANEST_RUNNER_REGISTRATION_TOKEN unset",
            ...
            match client.register(&config.name, &label_names, &reg).await {
                Ok((runner_id, token)) => {
                    ...
                    if let Err(e) = state::save(&config.state_path, &s) {
                        eprintln!(
                            "octanest-runner: failed to persist state to {}: {e}",
                            config.state_path.display()
                        );
                        std::process::exit(1);
                    }
                ...
                Err(e) => {
                    eprintln!("octanest-runner: registration failed: {e}");
                    std::process::exit(1);
                }
            }
        }
