//! SSH host key load/generate (Ed25519) under `OXIDEAN_SSH_HOST_KEY_DIR`.
...
/// Resolve host-key directory (`OXIDEAN_SSH_HOST_KEY_DIR`, default `var/ssh`).
pub fn host_key_dir() -> PathBuf {
    std::env::var("OXIDEAN_SSH_HOST_KEY_DIR")
        .map(PathBuf::from)
