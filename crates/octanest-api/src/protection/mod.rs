...
use octanest_core::{AppError, CommitStatusState, ProtectionBlockReasons};
use octanest_db::{BranchProtectionRuleRow, Database};
...
pub fn owner_name_from_git_dir(repos_dir: &Path, git_dir: &Path) -> Result<(String, String), String> {
    ...
    let rel = bare
        .strip_prefix(&repos)
        .map_err(|_| "GIT_DIR outside OCTANEST_REPOS_DIR".to_string())?;
