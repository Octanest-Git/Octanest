...
fn allowed_headers() -> [HeaderName; 4] {
    [
        ...
        header::ACCEPT,
        HeaderName::from_static("octanest-rpc-version"),
    ]
}
...
pub fn build_cors(env_name: &str, origins: Option<&str>) -> Result<CorsLayer, String> {
    ...
    let list = origins
        ...
        .filter(|s| !s.is_empty())
        .ok_or_else(|| {
            "OCTANEST_CORS_ORIGINS is required when OCTANEST_ENV is not development".to_string()
        })?;
    ...
    let parsed = parsed?;
    if parsed.is_empty() {
        return Err("OCTANEST_CORS_ORIGINS must list at least one origin".into());
    }
