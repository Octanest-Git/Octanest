...
//!
//! PR copies inherit preview `OCTANEST_PUBLIC_ORIGIN`; if that hostname is a
...
///
/// Prefer `OCTANEST_PUBLIC_ORIGIN` unless it is a stale Railway preview/PR host.
...
mod tests {
    ...
    fn prefers_railway_when_configured_host_stale() {
        ...
        unsafe {
            ...
            std::env::set_var(
                "RAILWAY_SERVICE_GATEWAY_URL",
                "gateway-octanest-pr-31.up.railway.app",
            );
        }
        assert_eq!(
            resolve_public_origin(),
            "https://gateway-octanest-pr-31.up.railway.app"
        );
        clear_origin_env();
    }
    ...
    fn keeps_custom_domain_when_railway_host_differs() {
        ...
        clear_origin_env();
        unsafe {
            std::env::set_var("OCTANEST_PUBLIC_ORIGIN", "https://octanest.jereko.dev");
            std::env::set_var(
                "RAILWAY_PUBLIC_DOMAIN",
                "gateway-production.up.railway.app",
            );
        }
        assert_eq!(resolve_public_origin(), "https://octanest.jereko.dev");
        clear_origin_env();
    }
    ...
    fn cors_replaces_stale_allowlist() {
        ...
        assert_eq!(
            resolved.as_deref(),
            Some("https://gateway-octanest-pr-31.up.railway.app")
        );
        clear_origin_env();
    }
}
