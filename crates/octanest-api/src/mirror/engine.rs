...
use octanest_db::{Database, RepositoryMirrorRow};
use octanest_git::{
...
const MIRROR_AUTHOR: &str = "Octanest Mirror <mirror@octanest.invalid>";
...
async fn create_mirror_pr(
    ...
    let _ = db
        .insert_pull(
            ...
            title,
            &format!(
                "Opened automatically by Octanest two-way mirroring.\n\nBase: `{base}` ← Head: `{head}`"
            ),
            &author_id,
            base,
            "",
...
async fn owner_slug_for_repo(
    db: &Database,
    repo: &octanest_db::RepositoryRow,
