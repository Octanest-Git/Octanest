...
use octanest_api::packages::quota::gc_unref_blobs;
use octanest_api::packages::store::{digest_of_bytes, put_blob};
...
async fn package_gc_keeps_shared_blob_and_removes_unref() {
    ...
    std::env::set_var("OCTANEST_PACKAGES_GC_GRACE_SECS", "0");
...
fn store_exists(packages_dir: &std::path::Path, digest: &str) -> bool {
    octanest_api::packages::store::blob_path(packages_dir, digest)
        .map(|p| p.exists())
        .unwrap_or(false)
}
