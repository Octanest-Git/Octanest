#!/usr/bin/env bash
# Octanest helper: ensure watermarks-remover HTTP service for a run, then stop
# it only if this script started it. Never kill a pre-existing listener.
#
# Usage:
#   octanest-watermarks-service.sh ensure          # start if needed; print STARTED|REUSED
#   octanest-watermarks-service.sh teardown        # stop only if we started
#   octanest-watermarks-service.sh run -- cmd...   # ensure → cmd → teardown
#
# Env:
#   WATERMARKS_SERVICE_URL   default http://127.0.0.1:8765
#   OCTANEST_ROOT            repo root (auto-detected from this script if unset)
#   WATERMARKS_CHECKOUT      default $OCTANEST_ROOT/tmp/watermarks-remover
...
OCTANEST_ROOT="${OCTANEST_ROOT:-$(cd "$SCRIPT_DIR/../../../.." && pwd)}"
CHECKOUT="${WATERMARKS_CHECKOUT:-$OCTANEST_ROOT/tmp/watermarks-remover}"
PIN_TAG="${WATERMARKS_PIN_TAG:-v0.7.0}"
STATE_DIR="${OCTANEST_ROOT}/tmp"
