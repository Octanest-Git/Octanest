...
describe("CloneBox SSH (GIT-03 / D-SSH-02 / D-SSH-06)", () => {
  it(
    "primary clone string is scp-style git@host:owner/repo.git (not ssh://)",
    async () => {
      render(CloneBox, {
        props: {
          owner: "ada",
  ...
  it(
    "shows Port hint when advertised SSH port is 2222",
    async () => {
      render(CloneBox, {
        props: {
          owner: "ada",
  ...
  it(
    "compact Add an SSH key CTA points to /settings/ssh-keys",
    async () => {
      render(CloneBox, {
        props: {
          owner: "ada",
      ...
      const cta = await waitFor(() =>
        screen.getByRole("link", { name: "Add an SSH key" }),
      );
