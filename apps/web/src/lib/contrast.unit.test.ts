...
describe("contrast helpers", () => {
  ...
  it("meets AA for deeper dark theme body and muted text", () => {
    ...
    // Charcoal canvas sits below the elevated card surface
    expect(relativeLuminance("#121212")).toBeLessThan(relativeLuminance("#1b1d21"));
  });
  ...
  it("keeps styles.css dark canvas charcoal and muted text AA", async () => {
    const fs = await import("node:fs");
    const path = await import("node:path");
    const url = await import("node:url");
    const cssPath = path.join(path.dirname(url.fileURLToPath(import.meta.url)), "../styles.css");
    const css = fs.readFileSync(cssPath, "utf8");
    expect(css).toMatch(/\.dark\s*\{[^}]*--background:\s*#121212/s);
