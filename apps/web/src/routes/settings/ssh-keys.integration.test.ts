...
describe("/settings/ssh-keys (GIT-04 / D-SSH-06 list)", () => {
  it(
    "list title SSH keys + empty hero No SSH keys + Add SSH key",
    async () => {
  ...
  it(
    "unverified: list visible with Add disabled + Verify your email to add an SSH key.",
    async () => {
      loaderData = {
        kind: "ready",
      ...
      expect(
        screen.getByText("Verify your email to add an SSH key."),
      ).toBeInTheDocument();
      await waitFor(() => {
        expect(screen.getByText("No SSH keys")).toBeInTheDocument();
      });
  ...
  it(
    "settings secondary nav Profile | Personal access tokens | SSH keys",
    async () => {
      const mod = await loadSshKeysModule();
      const SshKeysPage = (mod.SshKeysPage ?? mod.default) as unknown;
      const { container } = renderWithQueryClient(SshKeysPage);
  ...
  it(
    "list rows show SHA256 fingerprint",
    async () => {
...
describe("/settings/ssh-keys (GIT-04 / D-SSH-05 revoke)", () => {
  it(
    "revoke AlertDialog copy Revoke SSH key? / Keep key / Revoke key",
    async () => {
      listMock.mockResolvedValue({
        ok: true,
        data: [
      ...
      expect(
        screen.getByRole("button", { name: "Keep key" }),
      ).toBeInTheDocument();
      expect(
