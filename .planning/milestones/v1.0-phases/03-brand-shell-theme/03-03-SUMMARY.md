...
requires:
  - phase: 03-01
    provides: OctanestMark component, CVA Button/buttonVariants, full semantic token layer
provides:
...
key-decisions:
  - "Wrapped OctanestMark in a plain <div className=\"oct-rise\"> for the hero stagger instead of extending OctanestMark's props, since OctanestMark (owned by 03-01) only accepts size/className and adding a style prop would touch a file outside this plan's scope"
...
**Four-band editorial marketing landing (Hero → Dual-mode → Pillars → Closing CTA) with locked UI-SPEC copy, the shared 96px OctanestMark, brand-atmosphere gradients from semantic tokens, and exactly three reduced-motion-safe motions.**
...
- Removed the duplicate "Octanest" `<p>` above the hero headline; the 96×96 `OctanestMark` is now the only brand identifier in the hero (D-04)
...
## Decisions Made
- Wrapped `OctanestMark` in a `<div className="oct-rise" style={{ animationDelay: "0ms" }}>` rather than adding a `style` prop to the component itself, keeping this plan's changes scoped to `routes/index.tsx` only (03-01/03-02 own `octanest-mark.tsx`)
...
- `apps/web/src/routes/index.tsx` is fully migrated to semantic tokens, verified with `bun run --filter @octanest/web build` (exits 0), and matches every UI-SPEC Copywriting Contract string verbatim for the landing route
