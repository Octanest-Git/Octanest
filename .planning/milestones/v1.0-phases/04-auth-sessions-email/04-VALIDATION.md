...
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 04-01-T1 | 01 | 1 | AUTH-01/08 | T-04-02 | Username validation / reserved list | unit | `cargo test -p octanest-core --lib` | ✅ | ✅ green |
| 04-01-T2 | 01 | 1 | AUTH-01/02 | T-04-01 | Hash-only columns in schema | migration | `cargo test -p octanest-db --lib migration_parity` | ✅ | ✅ green |
| 04-01-T3 | 01 | 1 | AUTH-02/03 | T-04-01 | sessions.user_id + delete_all | integration | `cargo test -p octanest-db --test dialect_auth` | ✅ | ✅ green |
| 04-02-T1 | 02 | 1 | AUTH-09 | T-04-04 | LogSink only; no network | unit | `cargo test -p octanest-api --lib email::` | ✅ | ✅ green |
| 04-02-T2 | 02 | 1 | AUTH-10/11 | T-04-05 | Typed addresses; Resend User-Agent | unit | `cargo test -p octanest-api --lib email::` | ✅ | ✅ green |
| 04-03-T1 | 03 | 2 | AUTH-02 | T-04-09 | Argon2id PHC; no password logs | unit | `cargo test -p octanest-api --lib auth::password` | ✅ | ✅ green |
| 04-03-T2 | 03 | 2 | AUTH-02 | T-04-07/08/10 | Opaque HttpOnly cookie; SHA-256 token | unit | `cargo test -p octanest-api --lib auth::session` | ✅ | ✅ green |
| 04-04-T1 | 04 | 3 | AUTH-01/02/03 | T-04-11/12 | Generic login errors; mode gate | unit/lib | `cargo test -p octanest-api --lib` | ✅ | ✅ green |
| 04-04-T2 | 04 | 3 | AUTH-01/02/03 | T-04-13/14 | Cookie jar signup/session/logout | integration | `cargo test -p octanest-api --test auth_signup --test auth_session` | ✅ | ✅ green |
| 04-05-T1 | 05 | 4 | AUTH-02 | T-04-15/17 | WorkOS → Octanest session mint | unit | `cargo test -p octanest-api --lib auth::workos` | ✅ | ✅ green |
| 04-05-T2 | 05 | 4 | AUTH-02 | T-04-15/16 | OIDC PKCE + https issuer | unit | `cargo test -p octanest-api --lib auth::oidc` | ✅ | ✅ green |
| 04-06-T1 | 06 | 5 | AUTH-08 | T-04-19/20 | Server-chosen path; size/type allowlist | integration | `cargo test -p octanest-api --test profile_avatar` | ✅ | ✅ green |
| 04-06-T2 | 06 | 5 | AUTH-09/10/11 | T-04-21/22 | Admin gate; no secrets in RPC | integration | `cargo test -p octanest-api --test admin_auth_settings` | ✅ | ✅ green |
...
- [x] `crates/octanest-db/tests/dialect_auth.rs` — 04-01-T3
- [x] `crates/octanest-api` email unit tests — 04-02-T1/T2
- [x] `crates/octanest-api/tests/auth_signup.rs` — 04-04-T2
- [x] `crates/octanest-api/tests/auth_session.rs` — 04-04-T2
- [x] `crates/octanest-api/tests/profile_avatar.rs` — 04-06-T1
- [x] `crates/octanest-api/tests/admin_auth_settings.rs` — 04-06-T2
...
**Commands re-run green:** `octanest-core --lib`, `octanest-db --lib` + `dialect_auth`, `octanest-api --lib email::|auth::password|auth::session|auth::workos|auth::oidc`, `--test auth_signup|auth_session|profile_avatar|admin_auth_settings`, `apps/web bun run build`.
