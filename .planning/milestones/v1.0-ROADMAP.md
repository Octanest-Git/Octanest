...
- [x] **Phase 3: Brand Shell & Theme** - Oxidean mark, chrome, light/dark with system default (completed 2026-09-09)
...
- [x] **Phase 11.1: Quality Hardening** (INSERTED) - Tests, coverage gates, repo HI/chrome, forge e2e, GSD honesty ([issue #3](https://github.com/oxidean/oxidean/issues/3)) (plans 9/9; G-11.1-15 gap-closure 07–09 done)
...
- [x] `02-01-PLAN.md` — oxidean-db connection layer (dialect resolution, redaction, `DbPool`/`Database` connect)
...
**Goal**: The product UI reads as Oxidean — mark, colors, naming — with light/dark themes defaulting to system and user override
**Depends on**: Phase 1
...
- [x] `03-01-PLAN.md` — semantic token layer + CVA Button/Input/Select + shared squircle OxideanMark
...
- [x] `03-04-PLAN.md` — branded `/status` hero states + `Status · Oxidean` title
...
**Goal**: Oxidean Cloud feels open to the public while requiring email verification before privileged actions and supporting password reset
**Depends on**: Phase 4
**Requirements**: AUTH-04, AUTH-05, AUTH-12
**Success Criteria** (what must be TRUE):

  1. On Oxidean Cloud, signup requires no invite
  2. On Oxidean Cloud, unverified users cannot perform privileged actions (at minimum: create repository) until email is verified
...
**Success Criteria** (what must be TRUE):
  ...
  1. On self-host, when `OXIDEAN_ADMIN_EMAIL` and `OXIDEAN_ADMIN_PASSWORD` are both set, first boot creates that admin account
...
**Requirements**: Issue [#3](https://github.com/oxidean/oxidean/issues/3) (tests, bugfixes, hydration, CI coverage gates, repo HI refactor, GSD doc honesty)
**Success Criteria** (what must be TRUE):
...
- [x] 14-02-PLAN.md — Tracer: Batch + basic transfer into OXIDEAN_LFS_DIR
...
- [x] 14-07-PLAN.md — Compose OXIDEAN_LFS_DIR + operator docs
...
**Goal**: Every PR proves Compose health across supported databases, and the same images deploy as Oxidean Cloud on a container host
