...
**Migration:** `0014_packages` (postgres/sqlite/mysql) — confirmed; not 0012/0013
...
| -------- | -------- | ------ | ------- |
| `crates/octanest-db/migrations/*/0014_packages.sql` | Schema packages/versions/blobs/refs/quotas | ✓ VERIFIED | All three dialects; dialect test green |
...
| ---- | ---- | ------- | -------- | ------ |
| `dialect_packages.rs` | ~27 | Assert message says `"0012 must define {}"` while checking `0014_packages.sql` | ℹ️ Info | Misleading failure text only; migration id is correct |
...
Registry protocols (OCI/npm/generic), hybrid ACL∩PAT, session RPC list/delete, quotas/GC, owner packages UI, admin/tokens UI, docs, and `0014_packages` migration are present, wired, and backed by passing Rust tests. One D-PKG-11 gap blocks phase close: the repo packages route does not scope to the repository in the URL (backend list-by-repo works; UI does not use it). Fix that filter (and preferably strengthen the matching Vitest), then re-verify.
