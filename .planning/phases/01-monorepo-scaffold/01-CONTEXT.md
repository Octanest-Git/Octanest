...
- **D-02:** Rust crates for Phase 1: `octanest-api` (HTTP/WS server + RPC mount), `octanest-core` (shared domain/types), `octanest-db` (uniform DB adapter; Postgres default in Compose — full multi-dialect proof is Phase 2)
...
- **D-08:** `octanest-db` is the uniform adapter boundary so API code does not dialect-branch ad hoc
...
- **D-18:** Protocol versioning: header `Octanest-RPC-Version: 1`; server **rejects mismatch**; paths stay unversioned; **no multi-version routing** in Phase 1
- **D-19:** Browser: **explicit CORS** for local + Compose; credentials supported for future session cookies
- **D-20:** CORS policy: **dev = allow any origin**; **prod/Compose = require** `OCTANEST_CORS_ORIGINS` (comma-separated allowlist)
...
- **D-23:** Phase 1 slice: **credible marketing draft** on `/` (real Octanest positioning copy + mark in header); Search/Auth are **placeholders**; `/status` is **health-focused** live `system.health` only — **no echo UI**, **no status history persistence**
...
- `.env.example` shape for `OCTANEST_CORS_ORIGINS` and DB URLs
...
- Brand mark: `brand/octanest-mark.png` (blue/orange X on black)
...
### Brand
- `brand/octanest-mark.png` — primary mark for header/favicon until vector set exists
...
### Reusable Assets
- `brand/octanest-mark.png` — use in landing header / favicon scaffold
...
- `apps/web` consumes `@octanest/api-client` (or equivalent package name) via TanStack Query helpers
...
- Multi-DB operator proof (SQLite + Postgres + MySQL migrations/flows) — **Phase 2** (`octanest-db` adapter lands in Phase 1 for shape only as needed)
