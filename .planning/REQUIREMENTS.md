...
- [x] **PLAT-01**: Operator can run the full Oxidean stack with Docker Compose locally
- [x] **PLAT-02**: Operator can deploy the same images/stack to a container host (e.g. Railway) as Oxidean Cloud
...
- [x] **AUTH-04**: On Oxidean Cloud, user must verify email before privileged actions (at minimum: create repository)
- [x] **AUTH-05**: On Oxidean Cloud, signup is open (no invite required) — *v1 note: after empty-instance bootstrap, local signup is governed by instance `allow_signup` (ENV `OXIDEAN_ALLOW_SIGNUP`, default false). Cloud deploys that want open signup set it true; no invite codes in v1.*
- [x] **AUTH-06**: Empty instance: if `OXIDEAN_ADMIN_EMAIL` and `OXIDEAN_ADMIN_PASSWORD` are both set, first boot creates that admin account (`system-administrator`, forced credential change on first visit)
...
- [x] **ACT-04**: Operator can register and run an official Oxidean runner image against an instance
...
- [x] **ACT-07**: Workflow jobs only run on registered runners; Oxidean Cloud does not sell managed runner minutes in v1
...
- [x] **BRAND-01**: Product UI uses the Oxidean mark (`brand/oxidean-mark.png`) and blue/orange brand direction
- [x] **BRAND-02**: Product is named and presented as Oxidean (not a “GitHub clone” or bare Octane)
...
| Forking Gitea/Forgejo as product identity | Oxidean is its own brand and direction |
...
| Invite-only cloud gate in v1 | Signup openness is `allow_signup` after bootstrap (cloud sets `OXIDEAN_ALLOW_SIGNUP=true` when open signup desired); invite codes deferred |
